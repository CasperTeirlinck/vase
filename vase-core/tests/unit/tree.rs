...
fn windows_lists_only_window_leaves() {
    let tree = Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, win(1)), leaf(1, Pane::Empty)] };
...
fn find_window_returns_the_holding_pane() {
    let tree = Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, win(1)), leaf(1, win(2))] };
...
fn split_pane_replaces_the_leaf_with_a_split_of_it_and_a_new_empty() {
    ...
    assert_eq!(result, Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, win(1)), leaf(7, Pane::Empty)] });
}
...
fn split_pane_recurses_to_find_the_target() {
    ...
    assert_eq!(
        ...
        Node::Split {
            dir: Dir::Horizontal,
            ratios: vec![0.5, 0.5],
            children: vec![leaf(0, win(1)), Node::Split { dir: Dir::Vertical, ratios: vec![0.5, 0.5], children: vec![leaf(1, win(2)), leaf(7, Pane::Empty)] },],
        }
    );
}
...
fn swap_leaves_swaps_payloads_keeping_ids() {
    let mut tree = Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, win(1)), leaf(1, Pane::Empty)] };
    swap_leaves(&mut tree, PaneId(0), PaneId(1));
    assert_eq!(tree, Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, Pane::Empty), leaf(1, win(1))] });
}

#[test]
fn select_stack_window_selects_and_returns_stack_id() {
    let mut tree = Node::Split { dir: Dir::Horizontal, ratios: vec![0.5, 0.5], children: vec![leaf(0, win(1)), Node::Stack { id: PaneId(9), items: vec![win(2), win(3), win(4)], selected: 0 }] };
...
fn stack_select_picks_nth_window_skipping_empty() {
    let stack = Node::Stack { id: PaneId(5), items: vec![win(1), Pane::Empty, win(2), win(3)], selected: 0 };
...
fn stack_move_reorders_and_clamps() {
    let stack = Node::Stack { id: PaneId(5), items: vec![win(1), win(2), win(3)], selected: 1 };
    let Node::Stack { items, selected, .. } = stack_move(stack.clone(), PaneId(5), 1).unwrap() else { panic!() };
