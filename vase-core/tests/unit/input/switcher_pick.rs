...
fn a_digit_picks_a_row_by_its_number() {
    let mut s = sw();
    assert_eq!(press(&mut s, KeyCode::Char('2')), Pick::Chosen(Row::Win(2)));
}
...
fn a_launch_row_is_selectable_but_unnumbered() {
    ...
    ch(&mut s, 'j');
    assert_eq!(press(&mut s, KeyCode::Return), Pick::Chosen(Row::Launch(7)), "reachable by ⏎");
    // …but a digit only ever addresses the one numbered row.
    assert_eq!(press(&mut s, KeyCode::Char('2')), Pick::Ignored);
}
...
fn a_half_typed_index_commits_on_tick() {
    ...
    let now = t0();
    assert_eq!(s.key(Key::plain(KeyCode::Char('1')), now), Pick::Ignored, "11, 12 still reachable");
...
fn a_navigation_key_abandons_a_half_typed_index() {
    ...
    let mut s = Switcher::new(rows);
    let now = t0();
    s.key(Key::plain(KeyCode::Char('1')), now);
    ch(&mut s, 'j');
