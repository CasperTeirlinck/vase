//! Rectangles and the pure layout function.
...
/// splitting `area` by each split's ratios along its direction.
pub fn layout(node: &Node, area: Rect, out: &mut Vec<(PaneId, Pane, Rect)>) {
    match node {
        ...
        Node::Stack { id, items, selected } => {
            // Reserve the top strip for the local bar; place only the selected item below.
            let content =
                Rect::new(area.x, area.y + STACK_BAR_H, area.w, area.h - STACK_BAR_H);
