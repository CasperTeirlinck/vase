...
// The stroke on every tab outline — #5c6370, matching the thin-cap separator
// color in the user's tmux status bar (lighter than the active-tab fill).
pub(crate) fn tab_border() -> Retained<NSColor> {
    theme_color(0.361, 0.388, 0.439, 1.0)
}
// A bright whitish outline marking a tab whose app has a focus-toggle hotkey.
