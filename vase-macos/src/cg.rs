...
fn dict_rect(dict: &CFDictionary, key: &str) -> Option<Rect> {
    ...
    Some(Rect::new(dict_f64(&bounds, "X")?, dict_f64(&bounds, "Y")?, dict_f64(&bounds, "Width")?, dict_f64(&bounds, "Height")?))
}
...
/// Every window across all Spaces (minus desktop elements); callers must filter.
...
fn windows_with(options: u32) -> Vec<WindowInfo> {
    ...
    // Each element is a CFDictionaryRef describing one window; the array itself is untyped (`*const c_void`), so downcast every entry through CFType.
    // https://developer.apple.com/documentation/coregraphics/1454852-cgwindowlistcopywindowinfo
    list.iter()
        .filter_map(|raw| {
            let dict = unsafe { CFType::wrap_under_get_rule(*raw as _) }.downcast::<CFDictionary>()?;
            Some(WindowInfo {
