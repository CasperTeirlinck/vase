//! macOS Accessibility backend for vase.
...
/// Set from any quit path — `⌥a q`, the kill chord, the menu-bar Quit item.
pub fn request_quit() {
...
// Menu-bar actions run on the AppKit side; they set a flag the run loop drains
// into a daemon call (the menu handler holds no daemon reference).
