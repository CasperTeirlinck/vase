...
impl Daemon {
            ...
            for (ti, tab) in screen.tabs.iter().enumerate() {
                ...
                match wins.len() {
                    ...
                    _ => {
                        ...
                        items.push((SwitchItem { target: SwitchTarget::Tab(si, ti), prefix: String::new(), icons, display, dim, current: false }, search));
        ...
        for (i, kid) in kids.iter().enumerate() {
            ...
            match kid {
                ...
                Child::Stack { wins, selected } => {
                    ...
                    items.push((SwitchItem { target: SwitchTarget::Window(*selected), prefix: g1.to_string(), icons, display, dim, current: false }, search));
                    let m = wins.len();
                    for (j, w) in wins.iter().enumerate() {
    ...
    fn win_item(&self, id: WindowId, prefix: String, dim: bool, focused: Option<WindowId>) -> SwitchItem {
        ...
        SwitchItem { target: SwitchTarget::Window(id), prefix, icons: vec![self.windows.app(id).to_string()], display: self.window_display(id, in_stack), dim, current: Some(id) == focused }
    }
}
