//! The menu-bar status item: a vase icon with a Settings / Quit menu.
...
define_class!(
    // SAFETY: NSObject has no subclassing requirements and StatusHandler holds
    // no Rust Drop state.
...
// ponytail: lockFocus is deprecated for resolution independence, but it's fine
// for a tiny solid template icon; switch to imageWithSize:flipped:drawingHandler:
// (needs block2) if it reads soft on Retina.
#[allow(deprecated)]
...
/// Add the vase status item (icon + Settings / Quit menu) to the menu bar. The
/// returned `NSStatusItem` must be kept alive for the program's lifetime.
pub fn install(mtm: MainThreadMarker) -> Retained<NSStatusItem> {
    ...
    // Don't let AppKit's menu-validation auto-disable our items (this app has no
    // key window / responder chain to validate against); enable them explicitly.
    ...
    // The menu item's target is a weak reference, so keep the handler alive for
